...
import static com.hand.hap.cloud.hpay.services.pcServices.union.sdk.SDKUtil.isEmpty;
...
//	private static X509Certificate encryptTrackCert = null;
	...
	public static void init() {
		addProvider();
		if (UnionConstants.TRUE_STRING.equals(SDKConfig.getConfig()
				.getSingleMode())) {
			// 单证书模式,初始化配置文件中的签名证书
			initSignCert();
		}
	...
	public static void initSignCert() {
		...
		try {
			...
			logger.info("InitSignCert Error", e);
		}
	}
	...
	public static void loadRsaCert(String certFilePath, String certPwd) {
		...
		try {
			...
			logger.info("LoadRsaCert Error", e);
		}
	}
...
//		}
	...
	private static X509Certificate initCert(String path) {
		...
		try {
			...
			logger.info("InitCert Error File Not Found", e);
	...
	private static void initValidateCertFromDir() {
		...
		try {
			...
			logger.info("LoadVerifyCert Error", e);
		} catch (FileNotFoundException e) {
			logger.info("LoadVerifyCert Error File Not Found", e);
		} catch (NoSuchProviderException e) {
			logger.info("LoadVerifyCert Error No BC Provider", e);
		} finally {
			if (null != in) {
	...
	public static PrivateKey getSignCertPrivateKey() {
		try {
			...
			logger.info("getSignCertPrivateKey Error", e);
			return null;
	...
	public static PrivateKey getSignCertPrivateKeyByThreadLocal(
		...
		try {
			...
			logger.info("获取[" + certPath + "]的签名证书的私钥失败", e);
			return null;
		}
	}
	...
	public static PrivateKey getSignCertPrivateKeyByStoreMap(String certPath,
		...
		try {
			...
			logger.info("getSignCertPrivateKeyByStoreMap Error", e);
			return null;
		} catch (UnrecoverableKeyException e) {
			logger.info("getSignCertPrivateKeyByStoreMap Error", e);
			return null;
...
//		}
	...
	private static String getCertIdIdByStore(KeyStore keyStore) {
		...
		try {
			...
			logger.info("getCertIdIdByStore Error", e);
			return null;
		}
	}
	...
	public static PublicKey getEncryptTrackCertPublicKey(String modulus,
		...
		if (SDKUtil.isEmpty(modulus) || SDKUtil.isEmpty(exponent)) {
			logger.info("[modulus] OR [exponent] invalid");
			return null;
		}
