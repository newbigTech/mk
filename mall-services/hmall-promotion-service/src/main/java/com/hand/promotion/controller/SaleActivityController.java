 ...
 * @author XinyangMei
 * @Title HelloWorld
 * @Description MAP促销活动操作接口
 * @date 2017/11/29 19:31
 */
@RestController
...
public class SaleActivityController {
    ...
    public ResponseData test(){
        return promotionActivityService.queryForZmall();
    }
    ...
    public ResponseData createActivity(@RequestBody Map map) {
    ...
    @ResponseBody
    public ResponseData enableActivity(@RequestBody List<Map<String, Object>> maps) {
    ...
    @ResponseBody
    public ResponseData inactiveActivity(@RequestBody List<Map<String, Object>> maps) {
        List<String> errList = new ArrayList<>();
        try {
            for (Map<String, Object> map : maps) {
                String activityId = map.get("id").toString();
