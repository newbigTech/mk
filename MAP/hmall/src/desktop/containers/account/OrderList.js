...
export default class OrderList extends Component{
  ...
  query() {
    ...
    fetch(`${odService}/order/queryForUserOrders/${page}/${PAGE_SIZE}`,{
      method: 'post',
...
// 取消订单
  closeOrder(order,noconfirm) {
    ...
    noconfirm ? cancel() : (() =>{
      ...
      fetch(`${odService}/order/updateOrderCanceled/${orderId}`,{
        method: 'POST',
        headers: Hmall.getHeader()
      })
      .then(Hmall.convertResponse('json', this))
  ...
  orderRecycle(order) {
    if(confirm('确认删除？')){
      ...
      page==0 ? page=1:page;
      this.forceUpdate();
      fetch(`${odService}/order/updateOrderRecovered/${_orderId}`,{
        method: 'POST',
        headers: Hmall.getHeader({
          'Content-Type': 'application/json'
        }),
  ...
  getAfterMarketDetails(e, detail, oid) {
    ...
    if(!classList.contains('hover')){
      ...
      if(!init_aftermarkets && !aftermarkets) {
        ...
        fetch(`${odService}/order/queryForReturnId/${oid}/${productDetailInfo.productId}`,{
          headers: Hmall.getHeader()
        })
            .then(Hmall.convertResponse('json',this))
            .then(json => {
  ...
  getProductOperationBySign(detail, oid) {
    let { aftermarkets, init_aftermarkets, productDetailInfo, sign } = detail;
    switch(sign){
      case ORDER_SIGN[1]: return <p><Link to={`/account/order-center/order-list/view-refund.html?oid=${oid}&productId=${productDetailInfo.productId}`}>查看退款</Link></p>;
      case ORDER_SIGN[2]:
        let index = 0;
        return <div className="aftermarket" onMouseEnter={(e) => this.getAfterMarketDetails(e, detail, oid)}>
          <p>查看售后</p>
          <div>
            {
              init_aftermarkets ?
          ...
          <tr key={detailId} className="order-detail">
            ...
            </td>
            {
              ...
              <td rowSpan={row} className="center top">
                <p><b>￥{price.account}</b></p>
                <p>（含运费：￥{price.freight}）</p>
              </td>
            }
            {
              index == 0 &&
